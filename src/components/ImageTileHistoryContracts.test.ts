...
describe("Tile Swap history integration", () => {
  it("owns Undo/Redo in the current-puzzle crown for Tile Swap only", () => {
    ...
    expect(workspaceSource).toContain('historyCommand={puzzleId === "tile-swap" ? historyCommand : null}');
  });
  ...
  it("records completed swaps rather than tile selection as actions", () => {
    ...
    const historyPush = previewSource.indexOf(
    ...
    expect(historyPush).toBeGreaterThan(firstSelection);
    expect(
      previewSource.slice(firstSelection, historyPush),
    ).not.toContain("pushImageTileHistoryEntry");
  });
